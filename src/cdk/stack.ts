...
export class Stack extends cdk.Stack {
  constructor(scope: Construct, id: string, { environment, path, domain, project, priceClass, identity, ...props }: Props) {
    ...
    this.createEnvSpecificOrigin(distribution, bucket, originAccessIdentity, environment);
    this.createSubdomainRecords(distribution, zone, domain);
    this.createOutput(distribution, bucket, domain);
  }

  private getOriginAccessIdentity(identity?: string): cloudfront.OriginAccessIdentity {
    if (!identity) return new cloudfront.OriginAccessIdentity(this, 'OriginAccessIdentity');
    const oai = cloudfront.OriginAccessIdentity.fromOriginAccessIdentityId(this, 'OriginAccessIdentity', identity);
  ...
  private createEnvSpecificOrigin(
    ...
    const origin = this.getOrigin(bucket, originAccessIdentity, `/${environment}`);
